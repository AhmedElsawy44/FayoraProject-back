...
public class OpenRouterChatbotService : IChatbotService
{
    ...
    private string _model = "google/gemini-2.5-flash";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        requestMessage.Headers.Add("X-Title", "Fayora Travel");
...
";
        ...
        var requestBody = new
        {
            ...
            max_tokens = 1000 // Crucial to prevent credit/token limit errors for free keys on OpenRouter
        };
        ...
        try
        {
            for (int retry = 0; retry <= maxRetries; retry++)
            {
                ...
                cloneRequest.Headers.Add("HTTP-Referer", "https://fayora.com");
                cloneRequest.Headers.Add("X-Title", "Fayora Travel");
        ...
        {
            Console.WriteLine($"[OpenRouter Request Exception] {ex.Message}");
            return GetFallbackResponse("عذراً، حدث خطأ في الاتصال بخدمة OpenRouter. يرجى المحاولة مجدداً.");
        }

        if (response == null || !response.IsSuccessStatusCode)
        {
            var errContent = response != null ? await response.Content.ReadAsStringAsync(cancellationToken) : "Unknown";
            Console.WriteLine($"[OpenRouter Error Response] Code: {response?.StatusCode}, Body: {errContent}");
            return GetFallbackResponse("عذراً، لم نتمكن من الحصول على رد من خدمة OpenRouter.");
        }
        ...
        {
            Console.WriteLine($"[OpenRouter Parsing Exception] {ex.Message}");
        }
