...
public class VerifyDeletePhoneAccountCommandHandler(IUserRepository userRepository,
    ...
    IClientContextProvider clientContextProvider) : ICommandHandler<VerifyDeleteEmailAccountCommand, Result<Success>>
{
    ...
    public async Task<Result<Success>> Handle(VerifyDeleteEmailAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(context.Email)) return AuthErrors.PhoneNumberRequired;
        ...
        var vCode = await verificationCodeRepository.GetUserCodeAsync(user.Id, phoneNumber, CodePurpose.AccountDeletion, cancellationToken, isTracking: true);
