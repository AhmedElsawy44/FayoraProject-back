...
public class UpdateGuidePackageCommandValidator : AbstractValidator<UpdateGuidePackageCommand>
{
    public UpdateGuidePackageCommandValidator()
    {
        ...
        RuleFor(x => x.MainImageUrl)
            ...
            .Must(BeValidFileUrl).WithMessage("Main image URL must be a valid absolute URL starting with http or https.");

        RuleFor(x => x.VideoURL)
            .Must(BeValidFileUrl).WithMessage("Video URL must be a valid absolute URL starting with http or https.")
            .When(x => !string.IsNullOrWhiteSpace(x.VideoURL));

        RuleForEach(x => x.ImageURLs)
            .Must(BeValidFileUrl).WithMessage("Each image URL must be a valid absolute URL starting with http or https.");
