...
{
    public class LoginWithPhoneCommandHandler(IUserRepository userRepository, IUserTokenRepository userTokenRepository, IDeviceRepository deviceRepository, IUnitOfWork unitOfWork, IJwtService jwtService, ITokenHasher tokenHasher, IUserTokenService userTokenService, IPasswordHasher passwordHasher, IClientContextProvider clientContextProvider) : IRequestHandler<LoginWithPhoneCommand, Result<LoginWithPhoneResult>>
    {

        public async Task<Result<LoginWithPhoneResult>> Handle(LoginWithPhoneCommand request, CancellationToken cancellationToken)
        {
            var user = await userRepository.GetUserByPhoneAsync(request.PhoneNumber, new UserQueryOptions { IsReadOnly = false, IncludeRoles = true }, cancellationToken);
            ...
            return new LoginWithPhoneResult(user.Id, request.PhoneNumber, accessToken, refreshTokenString, jwtService.ExpiresIn);
        }
    }
}
